...
raw <- read_json("rdata/footgolf/nationals/nationals_scores_seniors.json", simplifyVector = T)
raw_unnested <- raw %>% unnest() %>% select(name, round1, round2)
...
scores_rd1 <- add_column(scores_rd1, round = rep(1, 306), .before = 'round1')
scores_rd2 <- add_column(scores_rd2, round = rep(2, 306), .before = 'round2')
# scores_rd3 <- add_column(scores_rd3, round = rep(3, 1800), .before = 'round3')

scores_rd1 <- add_column(scores_rd1, hole = rep(1:18, 17), .before = 'round1')
scores_rd2 <- add_column(scores_rd2, hole = rep(1:18, 17), .before = 'round2')
# scores_rd3 <- add_column(scores_rd3, hole = rep(1:18, 100), .before = 'round3')
...
scores_rd1 <- add_column(scores_rd1, par = rep(pars, 17))
scores_rd2 <- add_column(scores_rd2, par = rep(pars, 17))
# scores_rd3 <- add_column(scores_rd3, par = rep(pars, 100))
scores_rd1 <- add_column(scores_rd1, yds = rep(yds, 71))
scores_rd2 <- add_column(scores_rd2, yds = rep(yds, 71))
# scores_rd3 <- add_column(scores_rd3, yds = rep(yds, 100))
